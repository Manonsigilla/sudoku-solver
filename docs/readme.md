...
**Légende :** m = nombre de cases vides, d = taille du domaine (9), n = nombre de cases (81)
...
Les benchmarks réalisés via SQLite (`results.db`) montrent une progression **exponentielle** en faveur des algorithmes sophistiqués :

- **Force brute** : 30 secondes de timeout sur grilles difficiles (impraticable)
- **Backtracking** : 2-10 secondes selon la difficulté
- **Backtracking MRV** : 100-500ms (utile pour animations)
- **AC-3 seul** : ~10ms (mais échoue sur grilles difficiles)
- **AC-3 + MRV** : **< 1ms** (résout 100% des grilles valides)

### Algorithme le plus performant

**AC-3 + Backtracking MRV (approche Norvig) est clairement le gagnant** pour les raisons suivantes :

1. **Complétude :** Résout toutes les grilles valides
2. **Vitesse :** < 1ms même sur les grilles les plus difficiles
3. **Scalabilité :** Complexité O(d × n) dominée pour la propagation
4. **Élégance :** Combine le meilleur de deux mondes (déduction + recherche intelligente)

### Trade-offs

- **AC-3 seul** est plus rapide que AC-3+MRV sur les grilles faciles, mais échoue complètement sur les difficiles
- **Backtracking MRV** est un bon compromis si on prioritise la vitesse de calcul sans priorité absolue à la complétude
- **Force brute et Backtracking** ne doivent être utilisés que à titre éducatif

### Recommandation

Pour une application de production, **toujours utiliser AC-3 + MRV** car :

- Performance inégalée
- Aucune limite de difficulté
- Permet l'animation fluide (< 1ms)
- Standard reconnu en IA/recherche

---

*Cette analyse s'appuie sur le benchmarking intégré de l'application (SQLite) et les mesures de temps réel effectuées sur 5 grilles de difficultés variées.*